...
const requiredFiles = [
  'index.html', appPath, polishPath, editionLayerPath, languagePolishPath, publisherPath,
  'src/styles.css', 'src/polish.css', 'src/edition-layer.css',
  'public/icon.svg', 'public/manifest.webmanifest', 'public/sw.js',
  'public/data/news.json', 'public/data/ai_digest.json', 'public/data/topics.json',
  'public/data/edition.json', 'public/data/issues.json', 'public/data/storylines.json',
  'editions/reference/edition.yaml', 'docs/edition-protocol.md',
...
for (const reference of [
  './styles.css', './polish.css', './edition-layer.css', './editorial-app.js',
  './polish.js', './edition-layer.js', './language-polish.js', './manifest.webmanifest'
]) {
  if (!index.includes(reference)) throw new Error(`index.html is missing ${reference}`);
}
for (const contract of [
  'mobile-web-app-capable', 'apple-mobile-web-app-capable', 'id="app-status"',
  'role="status"', 'aria-atomic="true"', '持续观察的专业半月刊'
]) {
...
if (index.includes('./supabase-feedback.js')) throw new Error('the incomplete Supabase feedback client must not be loaded');
...
if (!Array.isArray(topics) || topics.length < 2) throw new Error('topics.json must contain the Edition channels');
...
for (const contract of [
  'body.is-scrolled .topbar', 'var(--paper) 50%', '.brand-name', 'font-family: var(--font-editorial)',
  'scrollbar-gutter: stable', 'content-visibility: auto', 'min-height: 44px', 'body.overlay-active',
  '@media (forced-colors: active)', '.mobile-search-backdrop', '.feed-list.list .article-tags', '.rail-card.prominent'
]) {
...
for (const selector of [
  ".app-shell[data-product-model='magazine-edition']", '.publication-nav', '.post-issue-intro',
  '.latest-edition-panel', '.edition-identity-card', '.storyline-rail', '.edition-archive',
  '.edition-panel', '@media (max-width: 620px)'
]) {
  if (!editionCss.includes(selector)) throw new Error(`edition-layer.css is missing ${selector}`);
}
if (editionCss.includes('box-shadow: var(--shadow-soft)')) throw new Error('formal Issue surface must not revert to dashboard-card elevation');
...
for (const contract of [
  "window.scrollY > 18", "classList.toggle('is-scrolled'", "observer.observe(appRoot, { childList: true })",
  'appRoot.inert = inert', 'trapSearchFocus', "setAttribute('aria-pressed'", "setAttribute('role', 'search')",
  "window.addEventListener('offline'", "window.addEventListener('online'", 'mobile-search-backdrop',
  'simplifyMasthead', 'simplifySidebar', 'simplifyFeed', 'simplifyRail', 'simplifyDrawer'
]) {
...
for (const contract of [
  'magazine-edition', '刊期之后的新变化', '最新刊期', '编辑台', '长期议题', '刊期与判断记录',
  'renderStorylineDrawer', 'renderArchiveDrawer', 'data-edition-action',
  "observer.observe(appRoot, { childList: true })", 'escapeEditionHtml'
]) {
...
for (const asset of [
  './editorial-app.js', './polish.js', './edition-layer.js', './language-polish.js',
  './styles.css', './polish.css', './edition-layer.css', './data/edition.json',
  './data/issues.json', './data/storylines.json'
]) {
  if (!serviceWorker.includes(asset)) throw new Error(`service worker is missing ${asset}`);
}
for (const contract of [
  'newsflow-editorial-v2.3.1', "event.request.mode === 'navigate'", 'networkFirst',
  "url.origin !== self.location.origin", 'event.waitUntil(networkUpdate'
]) {
  if (!serviceWorker.includes(contract)) throw new Error(`service worker is missing ${contract}`);
}
if (serviceWorker.includes('./supabase-feedback.js') || serviceWorker.includes('./data/supabase-config.json')) {
  throw new Error('service worker must not cache incomplete Supabase feedback assets');
}
if (serviceWorker.includes("'./app.js'")) throw new Error('service worker still caches the legacy frontend asset');

console.log(`NewsFlow checks passed: ${news.length} Signals, ${storylines.length} Storylines, ${issues.length} autonomous Issues, magazine-first hierarchy, issue-centred panels and resilient PWA caching.`);
