...
const renderEditionIdentity = (edition) => `
  <section class="sidebar-section edition-identity-section" data-edition-layer="identity">
    ...
    <div class="edition-identity-card">
      <div class="edition-identity-topline"><span>GitHub-native</span><span class="edition-auto"><span></span>自动出版</span></div>
...
const renderLatestIssue = (edition, issue) => {
  if (!issue) {
    return `<section class="latest-edition-panel" data-edition-layer="latest"><div class="issue-empty"><span>Latest Edition</span><h2>首期正在由自动出版流程准备</h2><p>Desk 会继续运行；正式刊期只收录达到 Edition 重要性规则的变化。</p></div></section>`;
  }
  ...
  return `<section class="latest-edition-panel" id="latest-edition" data-edition-layer="latest" aria-labelledby="latest-edition-title">
    <div class="issue-header">
      <div>
        <div class="issue-kicker">Latest Edition · Issue ${escapeEditionHtml(issue.issue_number)}</div>
...
const renderArchive = () => {
  ...
  return `<section class="edition-archive" data-edition-layer="archive" aria-labelledby="archive-title">
    <div class="archive-heading"><div><span class="section-label">Archive</span><h2 id="archive-title">刊期与判断记录</h2></div><p>每一期都是 Edition 文件、数据截止时间与自动出版结果的可追溯快照。</p></div>
    <div class="archive-list">
      ${issues.map((issue) => `<article class="archive-row"><span class="archive-number">${String(issue.issue_number).padStart(2, '0')}</span><div><h3>${escapeEditionHtml(issue.title)}</h3><p>${escapeEditionHtml(issue.standfirst)}</p></div><div class="archive-date">${escapeEditionHtml(formatEditionDate(issue.published_at))}<br>${issue.auto_generated ? 'Automatic' : 'Manual'}</div></article>`).join('') || '<p class="archive-empty">尚无已出版刊期。</p>'}
    </div>
  </section>`;
};
...
const applyEditionLayer = () => {
  ...
  if (brandStatus && !brandStatus.dataset.editionLayer) {
    ...
    brandStatus.innerHTML = '<span class="status-dot"></span>Autonomous edition';
  }
  ...
  if (!masthead.dataset.editionLayer) {
    ...
    if (kicker) kicker.textContent = 'Edition-defined intelligence · GitHub-native publishing';
    if (title) title.textContent = edition.name;
    if (deck) deck.textContent = edition.reader_promise;
    const issue = latestIssue();
    if (meta) meta.innerHTML = `Strong editor mode<br>${escapeEditionHtml(edition.owner?.editor || 'Repository editor')}<br>${issue ? `Issue ${escapeEditionHtml(issue.issue_number)}` : 'Pre-publication'}<br>1st / 15th automatic`;
  }
  ...
  if (leadLabel) leadLabel.textContent = 'Desk 首要信号';
  ...
  if (feedToolbar && !feedToolbar.previousElementSibling?.matches('[data-edition-layer="desk"]')) {
    feedToolbar.insertAdjacentHTML('beforebegin', '<div class="desk-intro" data-edition-layer="desk"><span class="section-label">Editorial Desk</span><p>持续运行的新信号。正式半月刊只采用达到 Edition 重要性门槛的内容。</p></div>');
  }
