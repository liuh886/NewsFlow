...
const renderCard = (item, index) => {
  const id = getId(item);
  const saved = state.bookmarks.has(id);
  return `<article class="article-card ${state.focusedIndex === index ? 'keyboard-focus' : ''}" id="signal-${encodeURIComponent(id)}" data-card-index="${index}" data-channel-id="${escapeHtml(item.channel_id || '')}">
...
app.addEventListener('click', (event) => {
  ...
  if (action === 'topic') { state.topic = value; state.date = 'all'; state.entity = ''; state.focusedIndex = -1; state.mobileOpen = false; }
    ...
    const editorialMode = document.querySelector('[data-action="open-editorial-office"]')?.dataset.newsflowRole;
    if (action === 'mobile-menu' && editorialMode === 'editor') {
